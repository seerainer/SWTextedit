...
public final class FindReplace {
    ...
    private final ModifyListener searchModify = _ -> enableButtons();

    /** Modify listener for the replace field. */
    private final ModifyListener replaceModify = _ -> enableButtons();

    /** Listener for the sensitive check button. */
    private final SelectionListener sensitive = widgetSelectedAdapter(_ -> enableButtons());

    /** Listener for the find button. */
    private final SelectionListener find = widgetSelectedAdapter(_ -> {
	if (findEntry()) {
    ...
    private final SelectionListener replacefind = widgetSelectedAdapter(_ -> {
	replace();
	if (findEntry()) {
	    info.setText(""); //$NON-NLS-1$
	} else {
    ...
    private final SelectionListener replace = widgetSelectedAdapter(_ -> {
	replace();
	enableButtons();
	text.setFocus();
	dialog.setFocus();
    ...
    private final SelectionListener replaceall = widgetSelectedAdapter(_ -> {
	int counter;
	...
	text.setFocus();
	dialog.setFocus();
	info.setText(new StringBuilder().append(counter).append(" ") //$NON-NLS-1$
		.append(configData.getLangRes().getString("find_replaced")).toString()); //$NON-NLS-1$
    });
    ...
    private final ShellListener shellFocus = shellActivatedAdapter(_ -> enableButtons());
